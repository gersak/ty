/**
 * Ty Icons Loader for Next.js
 * 
 * This script loads essential icons for the Ty components library.
 * It waits for the Ty components to be available (loaded via CDN) 
 * before registering the icon definitions.
 */
(function() {
  'use strict';
  
  console.log('🎨 Initializing Ty icons loader...');
  
  // Icon definitions - Lucide icon set
  const ICON_DEFINITIONS = {
    ...
    'settings': '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="3"/><path d="M12 1v6m0 6v6m11-7h-6m-6 0H1"/></svg>',
    ...
    'layout': '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><line x1="9" y1="3" x2="9" y2="21"/><line x1="3" y1="9" x2="21" y2="9"/></svg>',
    ...
    'click': '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 12l2 2 4-4"/><path d="M21 12c0 4.97-4.03 9-9 9s-9-4.03-9-9 4.03-9 9-9c2.29 0 4.38.85 5.98 2.26"/></svg>',
    ...
    'thumbs-up': '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 9V5a3 3 0 0 0-3-3l-4 9v11h11.28a2 2 0 0 0 2-1.7l1.38-9a2 2 0 0 0-2-2.3zM7 22H4a2 2 0 0 1-2-2v-7a2 2 0 0 1 2-2h3"/></svg>',
    'thumbs-down': '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10 15v4a3 3 0 0 0 3 3l4-9V2H5.72a2 2 0 0 0-2 1.7l-1.38 9a2 2 0 0 0 2 2.3zm7-13h2.67A2.31 2.31 0 0 1 22 4v7a2.31 2.31 0 0 1-2.33 2H17"/></svg>',
    ...
    'lock': '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><circle cx="12" cy="16" r="1"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>',
    'unlock': '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><circle cx="12" cy="16" r="1"/><path d="M7 11V7a5 5 0 0 1 9.9-1"/></svg>',
    ...
    'arrow-down': '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><polyline points="5,12 12,19 19,12"/></svg>',
    ...
    'bluetooth': '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="6.5,6.5 17.5,17.5 12,23 12,1 17.5,6.5 6.5,17.5"/></svg>',
    ...
    'laptop': '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4" y="3" width="16" height="10" rx="2" ry="2"/><line x1="2" y1="14" x2="22" y2="14"/><line x1="2" y1="20" x2="22" y2="20"/></svg>',
  ...
  // Initialize icons when Ty components are available
  function initializeIcons() {
    if (window.ty && window.ty.icons && typeof window.ty.icons.add === 'function') {
      try {
        // Register all icon definitions
        window.ty.icons.add(ICON_DEFINITIONS);
        
        const iconCount = Object.keys(ICON_DEFINITIONS).length;
        console.log(`✅ Successfully loaded ${iconCount} icons for Ty components`);
        console.log('📝 Available icons:', Object.keys(ICON_DEFINITIONS).sort().join(', '));
      ...
      console.log('⏳ Ty icons system not ready yet...');
      return false;
    }
  }
  
  // Retry mechanism with exponential backoff
  function startIconLoader() {
    let attempts = 0;
    const maxAttempts = 30; // Try for ~3 seconds max
    
    function tryLoad() {
      attempts++;
      
      if (initializeIcons()) {
        // Success! Icons are loaded
        return;
      }
      
      if (attempts < maxAttempts) {
        // Retry with exponential backoff (50ms, 100ms, 150ms, 200ms, 250ms, then 250ms max)
        const delay = Math.min(50 * attempts, 250);
        setTimeout(tryLoad, delay);
      } else {
        console.warn('⚠️ Failed to initialize Ty icons after', maxAttempts, 'attempts.');
        console.warn('Make sure the Ty components script is loaded before this icons script.');
        console.warn('Expected: window.ty.icons.add function should be available.');
      }
    }
  ...
  // Start loading icons when DOM is ready or immediately if DOM already loaded
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', startIconLoader);
  } else {
