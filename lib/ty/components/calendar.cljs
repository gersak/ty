    ...
    (.addEventListener nav "change"
                       (fn [^js event]
                         (let [detail (.-detail event)
                               month (.-month detail)
                               year (.-year detail)]
                           ;; Update internal state - preserve selection when navigating
                           (update-calendar-state! el {:display-month month
                                                       :display-year year})
                           ;; Update HTML attributes and form value
...
(defn update-form-value!
  ...
  (let [state (get-calendar-state el)
    ...
    ;; Note: Attribute sync removed to prevent recursion with :attr handler
    ;; The value attribute is managed by update-attributes-from-state! instead
    ))
