...
(defn render-day-cell
  ...
  (let [day-element (.createElement js/document "div")
    ...
    ;; Add click handler
    (.addEventListener day-element "pointerdown"
                       (fn [^js event]
...
(defn render!
  ...
  (let [root (wcs/ensure-shadow el)
    ...
    (when custom-css
      (let [adopted (.-adoptedStyleSheets root)]
        (when-not (some #(identical? % custom-css) adopted)
          (set! (.-adoptedStyleSheets root) (.concat adopted #js [custom-css])))))
    ...
    (let [calendar-container (.createElement js/document "div")]
      ...
      (println "CALENDAR LOCALE: " locale)
...
(def configuration
  {:observed [:locale] ; Minimal attribute watching
