...
(defn components->output-value
  "Convert internal components to appropriate output format.
   For datetime-local inputs, outputs YYYY-MM-DDTHH:mm format (no timezone).
   For date-only inputs, outputs YYYY-MM-DD format."
  ...
  (when (and components
    ...
    (if (and with-time? (:hour components) (:minute components))
      ;; Output datetime-local format (YYYY-MM-DDTHH:mm) - no timezone
      ;; This matches HTML datetime-local input expectations
      (let [{:keys [year month day hour minute]} components]
        (str year "-"
             (gstr/format "%02d" month) "-"
             (gstr/format "%02d" day) "T"
             (gstr/format "%02d" hour) ":"
             (gstr/format "%02d" minute)))
...
(defn init-component-state!
  ...
  (let [initial-state (parse-date-picker-attributes el)]
    ...
    (let [components (select-keys initial-state [:year :month :day :hour :minute])]
      (set! (.-value el) (components->output-value components (:with-time initial-state))))
    initial-state))
...
(defn update-component-state!
  "Update internal date picker state - with staging for with-time mode"
  [^js el updates & {:keys [force-sync] :or {force-sync false}}]
...
(defn enrich-delta
  "Process attribute changes and parse date values for comparison - following calendar pattern"
  [^js el delta]
