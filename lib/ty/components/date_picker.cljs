...
(declare render! update-form-value! set-form-value!)
...
(defn emit-change-event!
  ...
  (let [state (get-component-state el)
    ...
    (set-form-value! el output-value)
...
(defn set-form-value!
  "Set form value using ElementInternals with ISO date/datetime string"
  [^js el iso-value]
  (when-let [internals (get-form-internals el)]
    (let [element-name (.getAttribute el "name")]
      (if (and element-name iso-value)
        (.setFormValue internals iso-value)
        (.setFormValue internals "")))))

(defn update-form-value!
  "Update form value based on current output value - leverages existing logic"
  [^js el]
  (let [current-value (.-value el)] ; Already set by emit-change-event!
    (set-form-value! el current-value)))

;; === COMPONENT REGISTRATION ===

(wcs/define! "ty-date-picker"
  {:observed [:value :size :flavor :label :placeholder :required :disabled
              :clearable :format :locale :min-date :max-date :first-day-of-week :with-time :name] ; ← Add :name
   :props {:value nil} ; ← Add property watching for framework integration
           ...
           (when (= attr-name "value")
             (let [with-time? (= (value/get-attribute el "with-time") "true")
               ...
               (update-component-state! el (or components {:year nil
                                                           ...
                                                           :hour nil
                                                           :minute nil}))))
           ;; Update form value when attributes change
           (update-form-value! el)
           (render! el))
   :prop (fn [^js el delta]
           ;; Property changes also trigger form value update
           (update-form-value! el)
           (render! el))})
