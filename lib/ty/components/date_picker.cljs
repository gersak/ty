...
(defn update-component-state!
  "Update internal date picker state - following calendar pattern"
  [^js el updates]
  (let [current-state (get-component-state el)
        new-state (merge current-state updates)]
    (set! (.-tyDatePickerState el) new-state)

    ;; Auto-sync when components change (following calendar pattern)
...
(defn handle-dialog-click! [^js el ^js event]
  "Handle clicks on the dialog element (native backdrop click behavior)"
  (let [root (wcs/ensure-shadow el)
        dialog (.querySelector root ".calendar-dialog")]
...
(defn handle-outside-click! [^js el ^js event]
  "Handle clicks outside the date picker input (for when calendar is closed)"
