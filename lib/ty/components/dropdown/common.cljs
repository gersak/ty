...
(declare update-selection-display! sync-selected-option! get-options get-option-data select-option!)
...
(defn update-dropdown-state!
  ...
  (let [state (get-dropdown-state el)
    ...
    (when (contains? updates :current-value)
      (let [current-value (:current-value updates)]
        ;; Update component value attribute/property
...
(defn enrich-delta
  ...
  (let [{:keys [current-value]} (get-dropdown-state el)]
    (if-not (contains? delta "value")
      ...
      (let [new-value (parse-dropdown-value (get delta "value"))]
        (if (not= current-value new-value)
          (assoc delta :current-value new-value)
          delta)))))


(defn sync-dropdown-with-state!
  "Sync dropdown UI with current state (self-contained shadow root access)"
  [^js el state-updates]
  (when (contains? state-updates :current-value)
    ;; Update visual state based on new value
    ;; Update selection display
    (update-selection-display! el)
    ;; Sync selected option in dropdown
...
(defn select-option!
  ...
  (when option
    ...
    ;; Update display to hide placeholder and show selection
    (update-dropdown-state! el {:current-value (.-value option)})))
