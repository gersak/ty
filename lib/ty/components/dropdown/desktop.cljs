(ns ty.components.dropdown.desktop
  ...
  (:require
    ...
    [ty.scroll-prevention :as scroll]
    [ty.shim :as wcs]))
...
(defn close-dropdown!
  ...
  [^js el]
  ;; Unlock body scroll BEFORE closing dropdown
  (scroll/unlock-body-scroll!)
...
(defn open-dropdown!
  "Open dropdown using showModal() for top layer rendering"
  [^js el ^js shadow-root]
  (let [{:keys [disabled readonly searchable value]} (common/dropdown-attributes el)]
    (when-not (or disabled readonly)
      ...
      (let [dialog (.querySelector shadow-root ".dropdown-dialog")
        ...
        (when dialog
          ;; Calculate bottom-left anchored position
          (calculate-dropdown-position! el shadow-root)

          ;; Lock body scroll BEFORE opening dropdown
          (scroll/lock-body-scroll!)

          ;; Use showModal instead of show for top layer
          (.showModal dialog)
          (.add (.-classList dialog) "open")

          ;; Focus and clear input for searchable dropdowns
          (when (and input searchable)
            (set! (.-value input) "")
            ;; Focus input after dialog is open and positioned
            (js/setTimeout #(.focus input) 10))
