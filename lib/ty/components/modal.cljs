                     ...
                     (when (= (.-target e) dialog) ; Click on backdrop, not content
                       ...
                       (if (wcs/parse-bool-attr el "protected")
                         (when (js/confirm "You have unsaved changes. Are you sure you want to close?")
                           (scroll/unlock-body-scroll!) ; Restore scroll before closing
                           (dispatch-modal-event! el "ty-modal-close" #js {:reason "backdrop-click"})
                           (wcs/rm-attr! el "open"))
                         (do
                           (scroll/unlock-body-scroll!) ; Restore scroll before closing
                           (dispatch-modal-event! el "ty-modal-close" #js {:reason "backdrop-click"})
    ...
    (let [listener (fn [e]
                     (when (= (.-key e) "Escape")
                       ...
                       (if (wcs/parse-bool-attr el "protected")
                         (when (js/confirm "You have unsaved changes. Are you sure you want to close?")
                           (scroll/unlock-body-scroll!) ; Restore scroll before closing
                           (dispatch-modal-event! el "ty-modal-close" #js {:reason "escape-key"})
                           (wcs/rm-attr! el "open"))
                         (do
                           (scroll/unlock-body-scroll!) ; Restore scroll before closing
                           (dispatch-modal-event! el "ty-modal-close" #js {:reason "escape-key"})
  ...
  (when-let [close-button (.querySelector dialog ".close-button")]
    ...
    (let [listener (fn [e]
                     ...
                     (if (wcs/parse-bool-attr el "protected")
                       (when (js/confirm "You have unsaved changes. Are you sure you want to close?")
                         (scroll/unlock-body-scroll!) ; Restore scroll before closing
                         (dispatch-modal-event! el "ty-modal-close" #js {:reason "close-button"})
                         (wcs/rm-attr! el "open"))
                       (do
                         (scroll/unlock-body-scroll!) ; Restore scroll before closing
                         (dispatch-modal-event! el "ty-modal-close" #js {:reason "close-button"})
  ...
  (let [{:keys [open backdrop close-on-outside-click close-on-escape]} (modal-attributes el)
    ...
    (if open
      (when-not (.-open dialog)
        ;; Lock body scroll BEFORE opening modal
        (scroll/lock-body-scroll!)
        (.showModal ^js dialog)
        (dispatch-modal-event! el "ty-modal-open" #js {}))
      (when (.-open dialog)
        ;; Unlock body scroll BEFORE closing modal
