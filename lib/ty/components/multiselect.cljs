...
(defn update-component-state!
  ...
  (let [state (get-component-state el)
    ...
    (when (contains? updates :selected-values)
      (let [selected-values (:selected-values updates)]
        ;; Update component value attribute
        (.setAttribute el "value" (str/join "," selected-values))
        ;; Update form value for HTMX compatibility
        (set-form-value! el selected-values)
        (sync-selections-with-tags! el selected-values)
...
(defn parse-multiselect-value
  ...
  [value-str]
  (if (and value-str (not-empty value-str))
    (str/split value-str #",")
    nil))
    ...
    (when-let [tag (if (= (.toLowerCase (.-tagName target)) "ty-tag")
      ...
      (when-not (.hasAttribute tag "disabled")
        ...
        (let [tag-value (get-tag-value tag)
          ...
          ;; Get new selection state and update component state
          (let [selected-values (conj (get-selected-values el) tag-value)]
            ;; Update component state (handles component value, form value, etc.)
            (update-component-state! el {:selected-values selected-values})



            ;; Dispatch change event
            (let [action (if new-selected "add" "remove")]
