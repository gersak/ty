...
(defn get-tab-label
  "Get label content from a ty-tab element.
   Checks for slot='label' first, then label attribute."
  [^js tab-el]
  (let [label-slot (.querySelector tab-el "[slot='label']")]
    (if label-slot
      ;; Clone the label slot content
      (.-outerHTML label-slot)
      ;; Fall back to label attribute
      (or (.getAttribute tab-el "label") "Tab"))))
  ...
  (let [root (wcs/ensure-shadow el)
    ...
    (if (and existing-container existing-buttons existing-viewport)
      ...
      (do
        ...
        ;; Update tab buttons (they might have changed labels/disabled state)
      ...
      (do
        ...
        (set! (.-innerHTML root)
              (str "<div class=\"tabs-container\" data-placement=\"" placement "\">"
                   (render-tab-buttons tabs active-id)
