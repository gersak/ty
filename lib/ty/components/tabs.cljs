...
(defn has-custom-marker?
  "Check if user provided custom marker content via slot"
  [^js el]
  (some? (.querySelector el "[slot='marker']")))
...
(defn calculate-marker-position
  "Calculate marker position and dimensions based on active tab button.
   Returns map with :left, :top, :width, :height in pixels, or nil if button not found."
  [^js el ^js shadow-root active-id]
  (when-let [button (.querySelector shadow-root (str "[data-tab-id='" active-id "']"))]
    (when-let [buttons-container (.querySelector shadow-root ".tab-buttons")]
      (let [button-rect (.getBoundingClientRect button)
            container-rect (.getBoundingClientRect buttons-container)]
        {:left (- (.-left button-rect) (.-left container-rect))
         :top 0
         :width (.-width button-rect)
         :height (.-height button-rect)}))))

(defn update-marker!
  "Update marker wrapper position and dimensions to match active tab button."
  [^js el active-id]
  (let [shadow-root (.-shadowRoot el)
        marker (.querySelector shadow-root ".marker-wrapper")]
    (when marker
      (when-let [{:keys [left top width height]}
                 (calculate-marker-position el shadow-root active-id)]
        (.. marker -style (setProperty "left" (str left "px")))
        (.. marker -style (setProperty "top" (str top "px")))
        (.. marker -style (setProperty "width" (str width "px")))
        (.. marker -style (setProperty "height" (str height "px")))))))
...
(defn render-tab-buttons
  ...
  (let [buttons (str/join ""
    ...
    (str "<div class=\"tab-buttons\" role=\"tablist\">"
         ;; Marker wrapper - only include default if no custom marker provided
         "<div class=\"marker-wrapper\">"
         (when-not (has-custom-marker? tabs-el)
  ...
  (let [root (wcs/ensure-shadow el)
    ...
    (if (and existing-container existing-buttons existing-viewport)
      ...
      (do
        ...
        ;; Remove all existing buttons (keep marker wrapper intact)
        (doseq [button (array-seq (.querySelectorAll existing-buttons ".tab-button"))]
          (.remove button))

        ;; Generate and append new buttons after the marker wrapper
        (let [buttons-html (render-buttons-only el tabs active-id)
              temp-div (.createElement js/document "div")
              _ (set! (.-innerHTML temp-div) buttons-html)
              buttons (vec (array-seq (.-children temp-div)))]
          ;; Append each button after the marker
          (doseq [button buttons]
            (.appendChild existing-buttons button)))
