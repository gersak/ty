    ...
    [:div.ty-content.rounded-lg.p-6.mb-4
     [:ty-radio-group {:label "Plan"
      ...
      [:label.flex.items-center.gap-2.cursor-pointer
       [:ty-radio {:value "free"}] [:span "Free"]]
      [:label.flex.items-center.gap-2.cursor-pointer
       [:ty-radio {:value "pro"}] [:span "Pro"]]
      [:label.flex.items-center.gap-2.cursor-pointer
       [:ty-radio {:value "team"}] [:span "Team"]]]]
    (common/code-block
   ...
   [:section.mb-10
    ...
    [:div.ty-content.rounded-lg.p-6.mb-4
     [:ty-radio-group {:label "Theme"
                       ...
                       :value "auto"}
      [:label.flex.items-center.gap-2.cursor-pointer
       [:ty-radio {:value "light"}] [:span "Light"]]
      [:label.flex.items-center.gap-2.cursor-pointer
       [:ty-radio {:value "dark"}] [:span "Dark"]]
      [:label.flex.items-center.gap-2.cursor-pointer
       [:ty-radio {:value "auto"}] [:span "Auto"]]]]]
   ...
   [:section.mb-10
    ...
    [:div.ty-content.rounded-lg.p-6.mb-4.flex.flex-col.gap-4
     [:ty-radio-group {:label "Priority" :flavor "danger" :value "high"}
      [:label.flex.items-center.gap-2.cursor-pointer
       [:ty-radio {:value "low"}] [:span "Low"]]
      [:label.flex.items-center.gap-2.cursor-pointer
       [:ty-radio {:value "med"}] [:span "Medium"]]
      [:label.flex.items-center.gap-2.cursor-pointer
       [:ty-radio {:value "high"}] [:span "High"]]]
     [:ty-radio-group {:label "Status" :flavor "success" :value "ok"}
      [:label.flex.items-center.gap-2.cursor-pointer
       [:ty-radio {:value "ok"}] [:span "OK"]]
      [:label.flex.items-center.gap-2.cursor-pointer
       [:ty-radio {:value "warn"}] [:span "Warning"]]]]]
