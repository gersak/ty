(ns ty.site.views.components-index
...
(def category-groups
  [{:title "Action"
   ...
   {:title "Date & time"
    :ids [:ty.site.docs/date-picker
          :ty.site.docs/calendar
          :ty.site.docs/calendar-month]}
   {:title "Overlays"
    :ids [:ty.site.docs/modal
          :ty.site.docs/popup
          :ty.site.docs/tooltip]}
   {:title "Layout primitives"
    :ids [:ty.site.docs/tabs
          :ty.site.docs/wizard
          :ty.site.docs/scroll-container
          :ty.site.docs/resize-observer]}
   {:title "Visual"
    :ids [:ty.site.docs/icon]}])

(defn flat-components []
  (mapcat (fn [c]
            (if (= (:id c) :ty.site.docs/inputs)
              (:children c)
              [c]))
...
(defn component-row
  ...
  [:button.group.w-full.text-left.flex.flex-col.md:flex-row.items-start.md:items-center.gap-4.md:gap-8.py-6.px-5.md:px-7.cursor-pointer.transition-colors.duration-150
   ...
   [:div.flex-1.w-full.flex.items-center.justify-start.md:justify-end.overflow-hidden
