...
function listIcons() {
  if (!existsSync(ICONS_PATH)) {
    console.error('❌ Icons directory not found:', ICONS_PATH)
    return []
  }
...
function readIconMetadata(svgPath) {
  ...
  try {
    ...
    console.warn(`⚠️  Failed to read metadata for ${basename(svgPath)}:`, error.message)
    return null
  }
}
...
function generateTypeScript(icons) {
  ...
  for (const icon of icons) {
    ...
    if (usedIdentifiers.has(icon.identifier)) {
      ...
      console.warn(`⚠️  Skipping duplicate identifier: ${icon.identifier} (${icon.iconName})`)
      continue
    }
    ...
    for (const alias of icon.aliases) {
      ...
      if (usedIdentifiers.has(alias.identifier)) {
        console.warn(`⚠️  Skipping duplicate alias: ${alias.identifier} (${alias.name})`)
        continue
      }
      
      usedIdentifiers.add(alias.identifier)
      lines.push(`// Alias: ${alias.name} → ${icon.iconName}`)
  ...
  if (skippedIcons.length > 0) {
    console.log(`\n⚠️  Skipped ${skippedIcons.length} duplicate icons:`)
 ...
 */
function generate() {
  console.log('🎨 Lucide Icons Generator for TypeScript')
  console.log('==========================================\n')
